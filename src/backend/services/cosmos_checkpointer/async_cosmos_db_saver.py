...
class AsyncCosmosDBSaver(BaseCheckpointSaver):
    ...
    async def from_conn_info(
        ...
        try:
            ...
            yield AsyncCosmosDBSaver(client, db_name, container_name, write_container_name)
        finally:
            if client:
                client.close()
    ...
    async def aget_tuple(self, config: RunnableConfig) -> Optional[CheckpointTuple]:
        ...
        try:
            result = [item async for item in self.container.query_items(query=query)]
            if result:
                doc = result[0]
                config_values = {
                    "thread_id": thread_id,
                ...
                serialized_writes = [item async for item in self.write_container.query_items(query=serialized_writes_query)]
                pending_writes = [
                    (
                        write_doc["task_id"],
                        write_doc["channel"],
    ...
    async def alist(
        ...
        *,
        ...
        try:
            ...
            #for doc in result:
            async for doc in self.container.query_items(query=query):
                checkpoint = self.serde.loads_typed((doc["type"], doc["checkpoint"]))
    ...
    async def aput_writes(
        ...
        try:
            for operation in operations:
                await self.write_container.upsert_item(operation)
        except exceptions.CosmosHttpResponseError as e:
            print(f"An error occurred: {e}")
